(() => {
	...
	function renderLanguageOptions() {
		...
		if (!languages.length) {
			const empty = createPanelElement('div', 'theme-lang-empty', translate('languagePickerEmpty', 'No languages available'));
			langList.innerHTML = '';
	...
	function createLanguageOption(code, name, currentLang) {
		...
		if (code === currentLang) {
			...
			const badge = createPanelElement('span', 'theme-lang-current', translate('languageCurrentTag', 'Current'));
	...
	function getFocusableElements() {
		if (!langPanel) return [];
		return Array.from(langPanel.querySelectorAll(
