(function () {
  ...
  function bindAccentPanel() {
      ...
      accents.forEach((c, i) => {
        ...
        b.addEventListener('click', () => {
          try {
            ...
            panel.querySelectorAll('.accent-dot').forEach(d => d.classList.remove('active'));
    ...
    const outside = e => {
      if (panel && !panel.contains(e.target) && e.target !== themeBtn) hidePanel();
    };
    const startPress = () => {
    ...
    ['mouseup', 'mouseleave', 'touchend', 'touchcancel', 'click'].forEach(ev =>
      themeBtn.addEventListener(ev, clearPress)
    );
    cleanups.push(() => {
      ...
      ['mouseup', 'mouseleave', 'touchend', 'touchcancel', 'click'].forEach(ev =>
        themeBtn.removeEventListener(ev, clearPress)
      );
      hidePanel();
    });
  }
    ...
    if (cfg.enableAsciiPanel) {
      ...
      window.showAscii = () => {
        ...
        Object.assign(box.style, {
          ...
          right: '20px',
          maxWidth: '280px',
          background: 'linear-gradient(145deg,rgba(20,20,22,.85),rgba(30,32,36,.82))',
          color: '#eee',
          ...
          boxShadow: '0 4px 24px -6px rgba(0,0,0,.5),0 0 0 1px rgba(255,255,255,.07)',
          overflow: 'hidden',
        ...
        copyBtn.addEventListener('click', () => {
          navigator.clipboard && navigator.clipboard.writeText(ascii).then(() => toast('已复制'));
        });
  ...
  if (!window.restoreFeatures) {
    window.restoreFeatures = function (level = 1) {
      ...
      bindConfetti();
      // 猫咪恢复
      if (window.initMaomao && !document.hidden) {
        try {
          window.initMaomao();
        } catch (_) {}
      }
  ...
  if (!window.releaseMemory) {
    window.releaseMemory = function (level = 1) {
      ...
      window.__MEM_RELEASED__ = level;
      window.__MEM_RELEASED_LEVEL__ = level;
  ...
  (function idleManager() {
    ...
    const mark = () => {
      lastActive = Date.now();
      if (autoRestore && window.__MEM_RELEASED__) {
        // 用户回来了 -> 恢复
    ...
    function loop() {
      ...
      if (!releasedLevel1 && idle > delay) {
        if (!window.__UPDATE_LOCK__) {
          // 更新过程锁定不释放
          window.releaseMemory(1);
          releasedLevel1 = true;
          if (deepDelay > 0) {
            deepTimer = setTimeout(() => {
              if (
                window.__MEM_RELEASED__ &&
                Date.now() - lastActive > delay + deepDelay &&
                !window.__UPDATE_LOCK__
              )
