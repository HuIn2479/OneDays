    ...
    window.detachMaomao = function () {
      ...
      try {
        ...
        if (globalCatEl.__mmResetEvents && globalCatEl.__mmResetHandler) {
          globalCatEl.__mmResetEvents.forEach(ev =>
            window.removeEventListener(ev, globalCatEl.__mmResetHandler)
          );
        }
        if (globalCatEl.__mmVisListener)
          document.removeEventListener('visibilitychange', globalCatEl.__mmVisListener);
      } catch (_) {}
