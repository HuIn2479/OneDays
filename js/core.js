(function () {
  ...
  const START_AT = new Date(
    cfg.launchDate || (cfg.meta && cfg.meta.launchDate) || '2021-02-27T00:00:00+08:00'
  ).getTime();
  ...
  function renderRuntime() {
    ...
    if (t2)
      t2.textContent = `${pad(h)}${t('runtimeHour')}${pad(m)}${t('runtimeMinute')}${pad(s)}${t('runtimeSecond')}`;
  }
        ...
        if (e.isIntersecting) {
          ...
          if (!box.classList.contains('reveal-in')) {
            [...box.children].forEach(ch => {
              if (!ch.classList.contains('reveal-item')) ch.classList.add('reveal-item');
            });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    function createToast(opts) {
      if (!opts) return;
      const { text = '', duration = 2400, id, variant = 'accent', closable = false } = opts;
    ...
    if (!window.__NET_STATUS_BOUND__) {
      ...
      window.addEventListener('offline', () =>
        createToast({ text: t('networkOffline'), variant: 'danger', id: 'net-off', duration: 3000 })
      );
      window.addEventListener('online', () =>
        createToast({ text: t('networkOnline'), variant: 'success', id: 'net-on', duration: 2500 })
      );
    }
    ...
    (function initHitokoto() {
      ...
      const t = window.__I18N__?.t || (k => k);
      const done = () => {
        if (wrap.classList.contains('skeleton')) removeSkeleton(wrap);
        obs && obs.disconnect();
    ...
    if (cfg.enableKonami) {
      ...
      function konamiFire() {
        ...
        const t = window.__I18N__?.t || (k => k);
        createToast({
          text: cfg.konamiToastText || t('konamiEasterEgg'),
          id: 'konami-eg',
          variant: 'accent',
          duration: 3000,
        });
    ...
    if (cfg.enableTitleClicks) {
      ...
      if (title) {
        let clicks = [];
        let grayscale = false;
        const need = cfg.titleClickThreshold || 7;
        const win = cfg.titleClickWindow || 2000;
        function prune() {
          const now = Date.now();
        ...
        function toggle() {
          grayscale = !grayscale;
          document.documentElement.style.filter = grayscale ? 'grayscale(1)' : 'none';
          const t = window.__I18N__?.t || (k => k);
          createToast({
            text: grayscale ? t('grayscaleModeOn') : t('grayscaleModeOff'),
            variant: 'neutral',
            duration: 2000,
          });
        }
